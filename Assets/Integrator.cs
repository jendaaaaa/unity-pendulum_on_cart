...
abstract public class Integrator  {
	...
	abstract public void RatesOfChange (double[] x, double[] xdot, double t, double u);
	...
	public void EulerStep(double [] x, double t, double h, double u) {
		RatesOfChange(x,k1,t,u);
	...
	/// <param name="x">The values being integrated.</param>
	/// <param name="h">The time step.</param>
	public double RK4Step(double [] x, double t, double h, double u) {
		RatesOfChange (x,k1,t,u);
		for (int i = 0; i < nEquations; i++) {
			store [i] = x [i] + k1 [i] * h / 2.0;
		}
		RatesOfChange (store,k2,t,u);
		for (int i = 0; i < nEquations; i++) {
			store [i] = x [i] + k2 [i] * h / 2.0;
		}
		RatesOfChange (store,k3,t,u);
		for (int i = 0; i < nEquations; i++) {
			store [i] = x [i] + k3 [i] * h;
		}
		RatesOfChange (store, k4, t, u);
		for (int i = 0; i < nEquations; i++) {
	 ...
	 */
	public double abmStep(double [] x, double t, double h, double u) {
		abmRms2 = 0.0;
		if(abmSteps==0) {
			...
			RatesOfChange(dm3,ym3,t,u);
			t = RK4Step(ym2,t,h,u);
			RatesOfChange(dm2,ym2,t,u);
			for(int i=0;i<x.Length;i++) {
				x[i] = ym2[i];
			}
			abmSteps+=1;
			...
			RatesOfChange(k1,x,t,u);
			for(int i=0;i<x.Length;i++) {
				P[i] = x[i] + (h/24.0)*
					(55.0*k1[i]-59.0*dm1[i]+37.0*dm2[i]-9.0*dm3[i]);
			}
			RatesOfChange(dp1,P,t+h,u);
			abmRms2 = 0.0;
			for(int i=0;i<x.Length;i++) {
				store[i] = x[i];
